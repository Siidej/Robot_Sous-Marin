function robotInWorld = KinematicModel(robotState, robotInWorld)
global const
tspan = [0 const.dt];
...
robotInWorld.theta = ztheta(2);
    function xypsi_dot = H(xypsi, uh, v, wh, r)
        xypsi_dot = zeros(3,1);
        xypsi_dot(1) = uh*cos(xypsi(3)) - v*sin(xypsi(3)) - wh*cos(xypsi(3));
        xypsi_dot(2) = uh*sin(xypsi(3)) + v*cos(xypsi(3)) - wh*sin(xypsi(3));
        xypsi_dot(3) = r;
    end 
    function ztheta_dot = V(ztheta, uv,vv, w, q)
       ztheta_dot = zeros(2,1);
       ztheta_dot(1) = uv*sin(-ztheta(2)) + w*cos(ztheta(2)) + vv*sin(-ztheta(2));
       ztheta_dot(2) = q;
    end 
end
